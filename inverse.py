...
class Inverse:
    ...
    def output_matrix(self):
        ...
        if self.cols <= self.rows: 
            Label(self.frame_inverse_output, text='Output:', font=('arial', 10, 'bold')).grid(row=1, column=self.cols*2)
            for i in range(self.rows):
                for j in range(self.cols):
                    Label(self.frame_inverse_output, text=self.compute_inverse()[i], bd=5).grid(row=i+2, column=self.cols*2+1)
    ...
    def input_matrix(self):
        ...
        Button(self.gui_inverse_input,text="Enter", width=8, command=get_mat).grid(row=self.m_height.get()+10, column=1)
